import { Controller, Redirect, Get, UseGuards, Query, Req } from "@nestjs/common";
...
export class PowensController {
    ...
    try {
      const response = await lastValueFrom(
        this.httpService.post(
          'https://lperrenot-sandbox.biapi.pro/2.0/auth/token/access',
          {
            code,
            client_id: '70395459',
  ...
  async getTransactions(@Req() req: Request) {
    ...
    let userTransaction = await this.transactionsService.getUserTransactions(
      user.id,
      'ASC',
    );

    let apiUrl =
      'https://lperrenot-sandbox.biapi.pro/2.0/users/me/transactions?limit=1000';
